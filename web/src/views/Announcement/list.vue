  ...
  <div>
    ...
    <sticky :z-index="10" class-name="sub-navbar">
      <el-select style="margin-left: 10px;" v-model="page.from" filterable placeholder="请选择信息来源">
        <el-option v-for="item in college" :key="item.id" :label="item.name" :value="item.name"></el-option>
      </el-select>
    ...
    >
      ...
      <el-table-column sortable prop="title" label="标题" width="180"></el-table-column>

      <el-table-column sortable prop="from" label="来源" width="180"></el-table-column>
  ...
  methods: {
    deleteList() {
      ...
      this.$confirm("此操作将永久删除, 是否继续?", "提示", {
        ...
        .then(() => {
          postJson("/tea/announcement/deleteAll", this.multipleSelection).then(
            (response) => {
    ...
    async handleModifyStatus(id, status) {
      await getJson("/public/announcement/findById/" + id).then((response) => {
        if (response.data.status === 200) {
    ...
    querySearchAsync(queryString, cb) {
      ...
      const res = queryString
        ? temType.filter(this.createTypeFilter(queryString))
        : temType;
    ...
    querySearchAsyncTitle(queryString, cb) {
      getJson("/public/announcement/findTitle").then((response) => {
        this.temTitle = response.data.data;
      });
      const temTitle = this.temTitle;
      const res = queryString
        ? temTitle.filter(this.createTitleFilter(queryString))
        : temTitle;
    ...
    createTitleFilter(queryString) {
      return (temTitle) => {
        return (
          temTitle.value.toLowerCase().indexOf(queryString.toLowerCase()) !== -1
        );
      };
    },
